...
def get_admin_record(svc: Client, user_id: str) -> Optional[dict]:
    """Get the shield_admins row for this auth user."""
    res = svc.table("shield_admins").select("*, shield_organizations(id, name, tenant_domain, plan, active)") \
...
<style>
    ...
    .org-card { background:#1a3050; border:1px solid rgba(46,117,182,.25); border-radius:12px; padding:1rem 1.25rem; margin-bottom:.6rem; display:flex; justify-content:space-between; align-items:center; }
    ...
    .login-box { max-width:420px; margin:4rem auto; background:#1a3050; border:1px solid rgba(46,117,182,.3); border-radius:16px; padding:2.5rem; }
    ...
    with tab_in:
        ...
        if submitted:
            ...
            try:
                ...
                # Verify they have a shield_admins record
                svc = get_svc_client()
                user = getattr(res, "user", None)
                if svc and user:
                    ...
                    if not admin:
                        ...
                        return
                    # Update last_login
                    svc.table("shield_admins").update({"last_login": datetime.now(timezone.utc).isoformat()}) \
...
def render_super_admin(svc: Client):
    ...
    with tab_orgs:
        orgs = svc.table("shield_organizations").select("*, shield_admins(email, role, last_login)") \
        ...
        for org in (orgs.data or []):
            ...
            scans = svc.table("shield_scan_log").select("id", count="exact") \
...
<div class="org-card">
  <div>
    ...
    <div style="font-size:.75rem;color:#64748b;margin-top:.2rem;">Admins: {admin_list}</div>
            ...
            with c1:
                if st.button("View", key=f"view_org_{org['id']}"):
                    st.session_state["selected_org_id"]   = org["id"]
                    st.session_state["selected_org_name"] = org["name"]
                    st.rerun()
            with c2:
                toggle_label = "Deactivate" if org.get("active") else "Activate"
                if st.button(toggle_label, key=f"toggle_{org['id']}"):
                    svc.table("shield_organizations").update({"active": not org.get("active")}) \
                        .eq("id", org["id"]).execute()
                    st.rerun()

        if st.button("➕ New Organization"):
            st.session_state["show_new_org"] = True
        ...
        if st.session_state.get("show_new_org"):
            ...
            with st.form("new_org"):
                ...
                with o2:
                    org_domain = st.text_input("Tenant domain", placeholder="acmecorp.com")
                ...
                webhook_secret = f"clarivise-shield-{org_domain.replace('.', '-')}-{__import__('secrets').token_hex(8)}" if org_domain else ""
                if st.form_submit_button("Create", type="primary"):
                    if org_name and org_domain:
                        svc.table("shield_organizations").insert({
                            ...
                            "inbound_webhook_secret": webhook_secret,
                        }).execute()
    ...
    with tab_invite:
        st.markdown("**Invite an admin to an organization**")
        ...
        with st.form("invite_admin"):
            ...
            if st.form_submit_button("Send Invite", type="primary"):
                if invite_org and invite_email:
                    ...
                    # Create shield_admins record — they'll set password via Supabase Auth invite
                    ...
                    if existing.data:
                        ...
                        svc.table("shield_admins").insert({
                            ...
                            "role": invite_role, "invited_by": "shawn@ingotsolutions.com",
                        }).execute()
                        # Send Supabase Auth invite email
                        try:
                            anon = get_anon_client()
                            svc.auth.admin.invite_user_by_email(invite_email)
                            st.success(f"Invite sent to {invite_email}. They'll receive an email to set their password.")
                        except Exception as ex:
...
def render_org_dashboard(svc: Client, org_id: str, org_name: str, is_super: bool, user_email: str):
    with st.sidebar:
        ...
        if is_super:
            if st.button("← All Organizations"):
                ...
                st.rerun()
            st.divider()
        tab = st.radio("Navigate", ["Overview", "Scan Log", "Quarantine", "Allow/Block", "Admins", "Settings"],
                       label_visibility="collapsed")
        st.divider()
        role_label = "Super Admin" if is_super else "Admin"
    ...
    if tab == "Overview":
        render_overview(svc, org_id)
    elif tab == "Scan Log":
        render_scans(svc, org_id)
    elif tab == "Quarantine":
        render_quarantine(svc, org_id)
    elif tab == "Allow/Block":
        render_lists(svc, org_id)
    elif tab == "Admins":
        render_admins(svc, org_id, is_super)
    elif tab == "Settings":
        render_settings(svc, org_id)


# ── Overview ───────────────────────────────────────────────────────────────────
def render_overview(svc: Client, org_id: str):
    scans = svc.table("shield_scan_log").select("verdict, created_at") \
        .eq("org_id", org_id).order("created_at", desc=True).limit(1000).execute()
    rows = scans.data or []
    ...
    quarantine_pending = svc.table("shield_quarantine").select("id", count="exact") \
        .eq("org_id", org_id).eq("status", "pending").execute().count or 0

    threats = verdicts.get("PHISHING", 0) + verdicts.get("SUSPICIOUS", 0) + verdicts.get("SPAM", 0)
    threat_rate = round((threats / total) * 100) if total else 0

    c1, c2, c3, c4 = st.columns(4)
    for col, val, color, label in [
        (c1, str(total),            "#2E75B6", "Emails Scanned"),
        (c2, str(quarantine_pending), "#dc2626", "Quarantine Pending"),
        (c3, f"{threat_rate}%",     "#d97706", "Threat Rate"),
...
def render_scans(svc: Client, org_id: str):
    ...
    with c2:
        filter_verdict = st.selectbox("Verdict", ["All", "PHISHING", "SUSPICIOUS", "SPAM", "SAFE"])
    with c3:
        limit = st.selectbox("Show", [50, 100, 250], index=0)
...
def render_lists(svc: Client, org_id: str):
    ...
    for tab, table_name, label in [(tab1, "shield_allowlist", "allowlist"), (tab2, "shield_blocklist", "blocklist")]:
        with tab:
            items = svc.table(table_name).select("*").eq("org_id", org_id).order("created_at", desc=True).execute().data or []
            for item in items:
            ...
            with st.form(f"add_{label}"):
                a1, a2 = st.columns(2)
                with a1:
                    val = st.text_input("Email or domain", placeholder="sender@example.com or example.com", key=f"{label}_val")
                with a2:
                    typ = st.selectbox("Type", ["email", "domain"], key=f"{label}_type")
...
def render_admins(svc: Client, org_id: str, is_super: bool):
    ...
    for admin in admins:
        ...
        linked = "✅ Linked" if admin.get("auth_user_id") else "⚠️ Not linked"
        c1, c2 = st.columns([5, 1])
        with c1:
            st.markdown(f"**{admin['email']}** &nbsp; `{admin.get('role','admin')}` &nbsp;·&nbsp; Last login: {last} &nbsp;·&nbsp; {linked}")
        with c2:
            if is_super and not admin.get("is_super_admin"):
                if st.button("Remove", key=f"rm_admin_{admin['id']}"):
    ...
    with st.form("invite_org_admin"):
        ...
        if st.form_submit_button("Send Invite", type="primary"):
            if inv_email:
                existing = svc.table("shield_admins").select("id").eq("email", inv_email).eq("org_id", org_id).execute()
                if existing.data:
                    ...
                    try:
                        ...
                        st.success(f"Invite email sent to {inv_email}.")
                    except Exception:
                        st.success(f"Admin record created. Share the dashboard URL with {inv_email} to complete sign-up.")
                    st.rerun()


# ── Settings ───────────────────────────────────────────────────────────────────
def render_settings(svc: Client, org_id: str):
    org = svc.table("shield_organizations").select("*").eq("id", org_id).limit(1).execute().data
    if not org:
    ...
    with st.form("settings"):
        ...
        st.text_input("Inbound webhook secret", value=org.get("inbound_webhook_secret", ""), disabled=True,
                      help="Use this as the secret when configuring your M365 mail flow rule.")
        it_email  = st.text_input("IT security email", value=org.get("it_security_email", "") or "")
        threshold = st.selectbox("Quarantine threshold",
                                 ["PHISHING", "SUSPICIOUS"],
        ...
        custom_prompt = st.text_area("Custom AI instructions (optional)",
                                     ...
                                     help="Additional context passed to Claude on every email analysis for this org.")
...
def main():
    ...
    anon = get_anon_client()
    if not anon:
        st.error("Dashboard not configured — SUPABASE_ANON_KEY missing.")
        return
    ...
    svc = get_svc_client()
    if not svc:
        st.error("Dashboard not configured — SUPABASE_SERVICE_ROLE_KEY missing.")
        return

    # Load admin record
    admin = get_admin_record(svc, str(user.id))
    if not admin:
        st.error("No Shield admin account found for your email. Contact support.")
        if st.button("Sign out"):
            anon.auth.sign_out()
            st.session_state.clear()
            st.rerun()
    ...
    # Super admin — show org list or drill into selected org
    if is_super:
        ...
        # Regular tenant admin — scoped to their org only
        org = admin.get("shield_organizations")
        if not org:
            # Fallback: look up org directly
            org_res = svc.table("shield_organizations").select("id, name").eq("id", admin["org_id"]).limit(1).execute()
